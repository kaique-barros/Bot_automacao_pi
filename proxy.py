...
async def handle_client_hybrid(client_reader, client_writer):
    ...
    try:
        ...
        if first_byte == b'\x05':
            ...
            await handle_socks5(client_reader, client_writer, addr, first_byte)
...
async def handle_http(client_reader, client_writer, addr, first_byte):
    ...
    try:
        ...
        task1 = asyncio.create_task(tunnel(client_reader, dest_writer))
        task2 = asyncio.create_task(tunnel(dest_reader, client_writer))
